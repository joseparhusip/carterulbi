...
// Perbarui biaya jika ada permintaan POST              
...
<body>  
        ...
        <div class="table-responsive">  
            <table class="table">  
                ...
                <tbody>  
                    ...
                    <?php else: ?>  
                        <tr>  
                            <td colspan="11" class="no-data">Tidak ada pemesanan ditemukan.</td>  
                        </tr>  
                    <?php endif; ?>  
                </tbody>  
            </table>  
    ...
    <script>  
        function updatePriceManually(idPengantaran) {  
            const biayaInput = document.getElementById('biaya_' + idPengantaran);  
            const biayaValue = parseFloat(biayaInput.value.replace(/[^0-9.,]/g, '').replace(',', '.'));  
  
            // Logika untuk memperbarui biaya dapat ditambahkan di sini  
            ...
            fetch('detail_pemesanan.php', {  
                    ...
                    body: `idPengantaran=${idPengantaran}&field=biaya&value=${biayaValue}`  
                })  
                .then(response => response.json())  
                .then(data => {  
                    if (data.success) {  
                        biayaInput.value = 'Rp' + biayaValue.toLocaleString('id-ID', {  
